...
do
	...
	if [ $MULTISITE = "Y" -o $SITE = $SITE_INSTALL ]
		then
			...
			drush en --yes --uri=$SITE all_strong_arms dialogues floatbox migrate_committee migrate_constituency migrate_memberships migrate_politician migrate_user_revisions roles slider_item profiles pw_breadcrumb pw_committees pw_content_authoring_layout pw_permissions pw_taxonomies rate_voteas subsite_* trustees
