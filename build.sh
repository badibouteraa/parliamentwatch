...
do
	...
	if [ $MULTISITE = "Y" -o $SITE = $SITE_INSTALL ]
		then
			...
			drush en --yes --uri=$SITE floatbox migrate_committee migrate_constituency migrate_memberships migrate_party migrate_politician migrate_user_revisions profiles roles rate_voteas slider_item subsite_conf trustees
